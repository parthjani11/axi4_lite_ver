class axi_base_seq extends uvm_sequence #(axi_item, axi_item);
  ...
  task print_values();
    `uvm_info("SEQ",
      $sformatf("AWADDR=%0d, AWPROT=%0b, AWVALID=%0d, WDATA=%0d, WSTRB=%0b, WVALID=%0d, ARADDR=%0d, ARPROT=%0b, ARVALID=%0d, RREADY=%0d",
      req.AWADDR, req.AWPROT, req.AWVALID, req.WDATA, req.WSTRB,
      req.WVALID, req.ARADDR, req.ARPROT, req.ARVALID, req.RREADY),
      UVM_MEDIUM)
  endtask

  task rand_on_resp(axi_item tr);
    if(rsp==null) begin
      if(req.randomize());
      else
...
class axi_rd_only_always extends axi_base_seq;

  `uvm_object_utils(axi_rd_only_always)

  function new(string name="axi_rd_only_always");
    super.new(name);
  endfunction
  ...
  task set_arvalid_1(axi_item tr);
    req.AWVALID=1'b0;
    req.WVALID=1'b0;
    req.ARVALID=1'b1;
  endtask
...
class axi_wr_only_always_simul extends axi_base_seq;

  `uvm_object_utils(axi_wr_only_always_simul)

  function new(string name="axi_wr_only_always_simul");
    super.new(name);
  endfunction
  ...
  task set_awvalid_wvalid_1(axi_item tr);
    req.AWVALID=1'b1;
    req.WVALID=1'b1;
    req.ARVALID=1'b0;
  endtask
...
class axi_wr_rd_always_simul extends axi_base_seq;

  `uvm_object_utils(axi_wr_rd_always_simul)

  function new(string name="axi_wr_rd_always_simul");
    super.new(name);
  endfunction
  ...
  task set_arvalid_awvalid_wvalid_1(axi_item tr);
    req.AWVALID=1'b1;
    req.WVALID=1'b1;
    req.ARVALID=1'b1;
  endtask
...
class axi_wr_only_always_1_by_1 extends axi_base_seq;

  bit c1,c2;

  `uvm_object_utils(axi_wr_only_always_1_by_1)

  function new(string name="axi_wr_only_always_1_by_1");
    super.new(name);
  endfunction
